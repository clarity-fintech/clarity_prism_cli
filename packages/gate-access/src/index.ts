...
function normalizeGateCode(raw: string): string {
  return raw.trim().toUpperCase().replace(/\s+/g, "");
}

export function formatGateCode(bytes: Uint8Array): string {
...
async function pbkdf2Derive(master: string): Promise<Uint8Array> {
...
export async function deriveGateAccessPassword(master: string): Promise<string> {
  if (!master.trim()) throw new Error("master secret required");
...
export async function digestGateAccessPassword(code: string): Promise<string> {
...
export async function verifyGateAccessPassword(
  code: string,
