...
export function registerPrism(program) {
    ...
    prism
        ...
        for (const line of result.reasoning) {
            if (line.includes("FAILED"))
                console.log(theme.risk(line));
            else
                step(line);
        }
        done(result.passed ? "VALIDATION PASSED" : "VALIDATION FAILED");
        console.log(JSON.stringify({ passed: result.passed, hash: result.event.content_hash }, null, 2));
    });
    prism
        .command("trace")
        ...
        for (const e of events) {
            console.log(`${theme.dim(e.timestamp)} ${theme.prism(e.type)} ${e.intent ?? ""} ${e.content_hash.slice(0, 8)}`);
        }
    });
    prism
        .command("stats")
    ...
    prism
        ...
        const estimate = {
            ...
            helix_fee_bps: 8,
            por_confidence: pred.confidence ?? 0.92,
            lane: "helix_internal",
        };
        done("ESTIMATE READY");
