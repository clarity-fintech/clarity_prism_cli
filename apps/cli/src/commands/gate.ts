...
import {
  deriveGateAccessPassword,
  digestGateAccessPassword,
} from "@clrt/gate-access";
...
function resolveMaster(opts: { master?: string }): string {
  ...
  throw new Error(
    "master secret required — pass --master <secret>, set CLRTY_GATE_MASTER in .env, or export CLRTY_GATE_MASTER"
  );
}
...
export function registerGate(program: Command): void {
  ...
  gate
    .command("password")
    .description(
      "Derive personal gate access code from your master secret (single source command)"
    )
    ...
    .action(async (opts: { master?: string; digest?: boolean }, cmd) => {
      const flags = parseGlobalFlags(cmd.parent?.parent?.opts() ?? {});
      header("GATE", "prism");
